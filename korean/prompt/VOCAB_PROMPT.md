...
- Mỗi batch nên phân bổ rõ source lane: `news_formal` (báo chí, chính luận, hành chính, bình luận), `native_spoken` (khẩu ngữ đời thường/công sở có sắc thái), hoặc `slang_online` (từ lóng, 신조어, meme, cách nói trên mạng còn đang hoạt động). Ghi source lane trong `context` hoặc metadata ẩn.
- Ưu tiên những từ khó, đa nghĩa, có hàm ý đánh giá, discourse hoặc register rõ rệt. Không đưa các từ thông dụng trực tiếp như target chỉ để lấp đủ 15 mục; nếu không có độ khó hoặc sắc thái đáng học, hãy giữ chúng trong passage làm supporting language.
